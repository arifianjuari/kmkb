...
<div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
    <div class="px-4 py-6 sm:px-0">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-bold text-gray-900">{{ __('Add New Allocation Map') }}</h2>
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-lg">
            ...
            <div class="px-4 py-5 sm:p-6">
                <form action="{{ route('allocation-maps.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-12">
                        <div class="col-span-12 md:col-span-6">
                            ...
                            <div class="mt-1">
                                <select id="source_cost_center_id" name="source_cost_center_id" required class="py-2 px-3 block w-full border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                            ...
                            <div class="mt-1">
                                <select id="allocation_driver_id" name="allocation_driver_id" required class="py-2 px-3 block w-full border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                                    ...
                                    @foreach($allocationDrivers as $driver)
                                        <option value="{{ $driver->id }}" {{ old('allocation_driver_id') == $driver->id ? 'selected' : '' }}>
                        ...
                        <div class="col-span-12 md:col-span-6">
                            ...
                            <div class="mt-1">
                                <input type="number" id="step_sequence" name="step_sequence" value="{{ old('step_sequence', $nextStep) }}" required min="1" class="py-2 px-3 block w-full border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                                @error('step_sequence')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror

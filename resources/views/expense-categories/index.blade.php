                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach($expenseCategories as $category)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            <a href="{{ route('expense-categories.show', $category) }}" class="inline-flex items-center px-3 py-1.5 border border-gray-300 text-xs font-medium rounded text-gray-700 bg-white hover:bg-gray-50">
                                                {{ __('View') }}
                                            </a>
                                            <a href="{{ route('expense-categories.edit', $category) }}" class="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded text-white bg-indigo-600 hover:bg-indigo-700 ml-2">
                                                {{ __('Edit') }}
                                            </a>
                                            <form action="{{ route('expense-categories.destroy', $category) }}" method="POST" class="inline ml-2">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded text-white bg-red-600 hover:bg-red-700" onclick="return confirm('{{ __('Are you sure you want to delete this expense category?') }}')">
                                                    {{ __('Delete') }}
                                                </button>
                                            </form>
                                        </td>

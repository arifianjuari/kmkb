...
<div class="max-w-7xl mx-auto">
    <div>
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-bold text-gray-900">{{ __('Edit Cost Reference Item') }}</h2>
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-lg">
            <div class="px-4 py-5 sm:px-6 border-b border-gray-200">
                <h3 class="text-lg leading-6 font-medium text-gray-900">{{ __('Cost Reference Item Details') }}</h3>
            </div>
            <div class="px-4 py-5 sm:p-6">
                <form action="{{ route('cost-references.update', $costReference) }}" method="POST">
                    @csrf
                    @method('PUT')
                            ...
                            <div class="mt-1">
                                @if(isset($uoms) && $uoms->count() > 0)
                                    <select id="unit_of_measurement_id" name="unit_of_measurement_id" required class="py-2 px-3 block w-full border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-biru-dongker-700 focus:border-biru-dongker-700">
                                        ...
                                        @foreach($uoms as $uom)
                                            <option value="{{ $uom->id }}" {{ old('unit_of_measurement_id', $costReference->unit_of_measurement_id) == $uom->id ? 'selected' : '' }}>
                                    ...
                                    @error('unit_of_measurement_id')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                    @if($costReference->unit && !$costReference->unit_of_measurement_id)
                                        <p class="mt-1 text-xs text-yellow-600">Legacy: {{ $costReference->unit }}</p>
                                    @endif
                        ...
                        <div class="col-span-12 md:col-span-6">
                            ...
                            <div class="mt-1">
                                <select id="category" name="category" class="py-2 px-3 block w-full border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-biru-dongker-700 focus:border-biru-dongker-700">
                                    ...
                                    <option value="barang" {{ old('category', $costReference->category) == 'barang' ? 'selected' : '' }}>{{ __('Obat/BHP') }}</option>
                                    <option value="tindakan_rj" {{ old('category', $costReference->category) == 'tindakan_rj' ? 'selected' : '' }}>{{ __('Tindakan Rawat Jalan') }}</option>
                                    <option value="tindakan_ri" {{ old('category', $costReference->category) == 'tindakan_ri' ? 'selected' : '' }}>{{ __('Tindakan Rawat Inap') }}</option>
                                    <option value="laboratorium" {{ old('category', $costReference->category) == 'laboratorium' ? 'selected' : '' }}>{{ __('Laboratorium') }}</option>
                                    <option value="radiologi" {{ old('category', $costReference->category) == 'radiologi' ? 'selected' : '' }}>{{ __('Radiologi') }}</option>
                                    <option value="operasi" {{ old('category', $costReference->category) == 'operasi' ? 'selected' : '' }}>{{ __('Operasi') }}</option>
                                    <option value="kamar" {{ old('category', $costReference->category) == 'kamar' ? 'selected' : '' }}>{{ __('Kamar') }}</option>
                                </select>
                                @error('category')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror

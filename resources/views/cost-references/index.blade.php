        ...
        <div class="flex justify-between items-center mb-6">
            ...
            <div class="flex items-center space-x-2">
                <form method="GET" action="{{ route('cost-references.index') }}" class="flex items-center space-x-2">
                    ...
                    @if($search)
                        <a href="{{ route('cost-references.index') }}" class="inline-flex items-center px-3 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                            {{ __('Clear') }}
                        </a>
                    @endif
                </form>
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @foreach($costReferences as $reference)
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                                <a href="{{ route('cost-references.show', $reference) }}" class="inline-flex items-center px-3 py-1.5 border border-gray-300 text-xs font-medium rounded text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                                                    {{ __('View') }}
                                                </a>
                                                <a href="{{ route('cost-references.edit', $reference) }}" class="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 ml-2">
                                                    {{ __('Edit') }}
                                                </a>
                                                <form action="{{ route('cost-references.destroy', $reference) }}" method="POST" class="inline ml-2">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500" onclick="return confirm('{{ __('Are you sure you want to delete this cost reference?') }}')">
                                                        {{ __('Delete') }}
                                                    </button>
                                                </form>
                                            </td>

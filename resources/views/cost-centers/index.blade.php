                ...
                @if(isset($groupedByDivision) && ($viewMode ?? 'tree') === 'tree')
                    ...
                    <script>
                        ...
                        document.addEventListener('DOMContentLoaded', function() {
                            // Hide all non-root cost center rows (rows with a parent_id)
                            const allRows = document.querySelectorAll('tr.cost-center-row');
                            allRows.forEach(row => {
...
<div id="importModal" class="hidden fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg shadow-xl max-w-md w-full mx-4">
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
            <h3 class="text-lg font-medium text-gray-900">Import Cost Centers</h3>
        ...
        <form action="{{ route('cost-centers.import') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="px-6 py-4">
                <div class="mb-4">
                    ...
                    <a href="{{ route('cost-centers.template') }}" class="inline-flex items-center text-sm text-biru-dongker-800 hover:text-biru-dongker-900">

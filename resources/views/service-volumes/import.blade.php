...
<div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
    <div class="px-4 py-6 sm:px-0">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-bold text-gray-900">{{ __('Import Service Volumes') }}</h2>
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-lg">
            <div class="px-4 py-5 sm:px-6 border-b border-gray-200">
                <h3 class="text-lg leading-6 font-medium text-gray-900">{{ __('Import Service Volumes from Excel') }}</h3>
                <p class="mt-1 text-sm text-gray-500">{{ __('Upload Excel file with format: Service Code, Tariff Class Code (optional), Total Quantity') }}</p>
            </div>
                ...
                <form action="{{ route('service-volumes.import.process') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-12">
                        ...
                        <div class="col-span-12">
                            ...
                            <div class="mt-1">
                                <input type="file" id="file" name="file" accept=".xlsx,.xls,.csv" required class="py-2 px-3 block w-full border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                                @error('file')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                                <p class="mt-2 text-sm text-gray-500">
                                    {{ __('Format file: Service Code | Tariff Class Code (optional) | Total Quantity') }}<br>

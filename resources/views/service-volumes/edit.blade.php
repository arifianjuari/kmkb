    ...
    <div>
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-lg">
            ...
            <div class="px-4 py-5 sm:p-6">
                <form action="{{ route('service-volumes.update', $serviceVolume) }}" method="POST">
                    @csrf
                    @method('PUT')
                            ...
                            <div class="mt-1">
                                <select id="category" name="category" class="py-2 px-3 block w-full border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-biru-dongker-700 focus:border-biru-dongker-700">
                                    ...
                                    @foreach($categoryOptions as $value => $label)
                                        <option value="{{ $value }}" {{ old('category', $serviceVolume->category) == $value ? 'selected' : '' }}>{{ __($label) }}</option>
                                    @endforeach
                                </select>
                                @error('category')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>

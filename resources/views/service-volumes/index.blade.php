...
<div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
    <div class="px-4 py-6 sm:px-0">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-bold text-gray-900">{{ __('Service Volumes') }}</h2>
            <div class="flex items-center space-x-2">
                ...
                <a href="{{ route('service-volumes.create') }}" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700">
                                ...
                                @foreach($serviceVolumes as $volume)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            <a href="{{ route('service-volumes.show', $volume) }}" class="inline-flex items-center px-3 py-1.5 border border-gray-300 text-xs font-medium rounded text-gray-700 bg-white hover:bg-gray-50">
                                                {{ __('View') }}
                                            </a>
                                            <a href="{{ route('service-volumes.edit', $volume) }}" class="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded text-white bg-indigo-600 hover:bg-indigo-700 ml-2">
                                                {{ __('Edit') }}
                                            </a>
                                            <form action="{{ route('service-volumes.destroy', $volume) }}" method="POST" class="inline ml-2">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded text-white bg-red-600 hover:bg-red-700" onclick="return confirm('{{ __('Are you sure you want to delete this service volume?') }}')">
                                                    {{ __('Delete') }}
                                                </button>
                                            </form>
                                        </td>

...
@section('title', 'Service Volume (Current) - Radiologi')
    ...
    <div class="bg-white shadow rounded-lg p-6">
        <form method="GET" class="grid gap-6 md:grid-cols-4">
            ...
            <div class="flex items-end space-x-2 md:col-span-4">
                ...
                <button type="submit"
                        formaction="{{ route('svc-current.radiologi.export') }}"
    ...
    <div class="bg-white shadow rounded-lg overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200">
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="text-lg font-semibold text-gray-900">Rekap Pemeriksaan Radiologi</h2>
                ...
                <div class="flex items-center space-x-2">
                    ...
                    <button id="sync-all-volumes" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 7v10c0 2.21 3.582 4 8 4s8-1.79 8-4V7M4 7c0 2.21 3.582 4 8 4s8-1.79 8-4M4 7c0-2.21 3.582-4 8-4s8 1.79 8 4"></path>
                        </svg>
                        Sync Semua (Ralan + Ranap)
                    </button>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    @empty
                        <tr>
                            <td colspan="17" class="px-4 py-6 text-center text-gray-500">Belum ada data radiologi untuk filter yang dipilih.</td>
                        </tr>
                    @endforelse
                </tbody>
                @if(count($radiologiData) > 0)
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    syncButton.addEventListener('click', function() {
        ...
        const items = [];
        selected.forEach(cb => items.push({ kode: cb.dataset.kode, nama: cb.dataset.nama, harga: cb.dataset.harga }));
        ...
        fetch('{{ route("svc-current.radiologi.sync") }}', {
            method: 'POST',
    ...
    syncVolumesButton.addEventListener('click', function() {
        ...
        fetch('{{ route("svc-current.radiologi.sync-volumes") }}', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content') },
    ...
    // Sync All Volumes (Ralan + Ranap) button handler
    const syncAllVolumesButton = document.getElementById('sync-all-volumes');
    syncAllVolumesButton.addEventListener('click', function() {
        if (!confirm('Apakah Anda yakin ingin sync SEMUA data radiologi (Ralan + Ranap) ke Service Volumes? Proses ini mungkin memakan waktu lebih lama.')) {
            return;
        }
        
        syncAllVolumesButton.disabled = true;
        syncAllVolumesButton.innerHTML = '<svg class="animate-spin w-4 h-4 mr-2" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg> Syncing...';
        
        fetch('{{ route("svc-current.radiologi.sync-all-volumes") }}', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
            },
            body: JSON.stringify({ year: {{ $year }}, kd_poli: '{{ $poli ?? '' }}' }),
            credentials: 'same-origin'
        })
        ...
        .finally(() => {
            syncAllVolumesButton.disabled = false;
            syncAllVolumesButton.innerHTML = '<svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 7v10c0 2.21 3.582 4 8 4s8-1.79 8-4V7M4 7c0 2.21 3.582 4 8 4s8-1.79 8-4M4 7c0-2.21 3.582-4 8-4s8 1.79 8 4"></path></svg> Sync Semua (Ralan + Ranap)';
        });
    });
});
</script>

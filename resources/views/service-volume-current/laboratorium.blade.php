...
document.addEventListener('DOMContentLoaded', function() {
    ...
    syncVolumesButton.addEventListener('click', function() {
        ...
        fetch('{{ route("svc-current.laboratorium.sync-volumes") }}', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
            },
            body: JSON.stringify({ items: itemsToSync, year: {{ $year }}, kd_poli: '{{ $poli ?? '' }}', status: '{{ $status ?? '' }}' }),
            credentials: 'same-origin'
        })
        .then(response => response.json())
        .then(data => {
    ...
    syncAllVolumesButton.addEventListener('click', function() {
        if (!confirm('Apakah Anda yakin ingin sync SEMUA data laboratorium (Ralan + Ranap) ke Service Volumes? Proses ini mungkin memakan waktu lebih lama.')) {
            return;
        }
        ...
        fetch('{{ route("svc-current.laboratorium.sync-all-volumes") }}', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')

...
<div class="max-w-3xl mx-auto">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-gray-900">{{ __('Edit Unit of Measurement') }}</h2>
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
        <div class="px-4 py-5 sm:p-6">
            <form action="{{ route('units-of-measurement.update', $unit) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <div class="mt-1">
                            <input type="text" id="code" name="code" value="{{ old('code', $unit->code) }}" required placeholder="e.g. m2, kg, tablet" class="py-2 px-3 block w-full border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-biru-dongker-700 focus:border-biru-dongker-700">
                            @error('code')
                    ...
                    <div>
                        ...
                        <div class="mt-1">
                            <input type="text" id="name" name="name" value="{{ old('name', $unit->name) }}" required placeholder="e.g. Meter Persegi, Kilogram" class="py-2 px-3 block w-full border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-biru-dongker-700 focus:border-biru-dongker-700">
                            @error('name')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div>
                        ...
                        <div class="mt-1">
                            <input type="text" id="symbol" name="symbol" value="{{ old('symbol', $unit->symbol) }}" placeholder="e.g. m², kg, tab" class="py-2 px-3 block w-full border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-biru-dongker-700 focus:border-biru-dongker-700">
                            @error('symbol')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mt-1">
                            <select id="category" name="category" required class="py-2 px-3 block w-full border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-biru-dongker-700 focus:border-biru-dongker-700">
                                @foreach($categories as $key => $label)
                                    <option value="{{ $key }}" {{ old('category', $unit->category) == $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                            @error('category')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div class="mt-1">
                            <select id="context" name="context" required class="py-2 px-3 block w-full border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-biru-dongker-700 focus:border-biru-dongker-700">
                                @foreach($contexts as $key => $label)
                                    <option value="{{ $key }}" {{ old('context', $unit->context) == $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                            @error('context')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div class="flex items-center">
                        ...
                        <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $unit->is_active) ? 'checked' : '' }} class="h-4 w-4 text-biru-dongker-700 focus:ring-biru-dongker-700 border-gray-300 rounded">

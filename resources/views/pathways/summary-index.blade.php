    ...
    <div>
        <div class="flex justify-between items-center mb-6 flex-wrap gap-4">
            <div class="flex items-center gap-3 flex-shrink-0">
                <h2 class="text-2xl font-bold text-gray-900 whitespace-nowrap">{{ __('Pathway Cost Summary') }}</h2>
                <button
                    type="button"
                    ...
                    onclick="const p = document.getElementById('summary-help'); if (p) { p.classList.toggle('hidden'); }"
                    aria-label="{{ __('What is Pathway Cost Summary?') }}"
                    title="{{ __('What is Pathway Cost Summary?') }}"
                >
                    i
                </button>
            </div>
        ...
        <div id="summary-help" class="mb-4 hidden text-xs text-gray-700 bg-biru-dongker-200 border border-biru-dongker-300 rounded-md p-3">
            <p class="mb-2">
                <span class="font-semibold">Pathway Cost Summary</span> menampilkan ringkasan biaya dan tarif untuk setiap pathway. Pilih pathway dari daftar untuk melihat breakdown biaya per langkah dan total estimasi biaya.
            </p>
            <ul class="list-disc list-inside space-y-1 ml-2">
                <li>Total estimasi biaya berdasarkan unit cost calculation</li>
                <li>Breakdown biaya per langkah pathway</li>
                <li>Perbandingan dengan versi unit cost yang berbeda</li>
                <li>Export ke PDF atau DOCX untuk dokumentasi</li>
            </ul>
        </div>
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-lg mb-6">
            <div class="px-4 py-5 sm:p-6">
                <form method="GET" action="{{ route('pathways.summary.index') }}" class="space-y-4">
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-4">
                        ...
                        @if($q || $status != 'all')
                            <div class="flex items-end">
                                <a href="{{ route('pathways.summary.index') }}" class="w-full inline-flex justify-center items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">
                                    {{ __('Clear') }}
                                </a>
                            </div>
                        @endif
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-lg">
            <div class="px-4 py-5 sm:p-6">
                <h3 class="text-lg font-medium text-gray-900 mb-4">{{ __('Select Pathway to View Summary') }}</h3>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('Unit Cost Version') }}</th>
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-6 py-2 whitespace-nowrap text-right text-sm font-medium">
                                            <div class="flex justify-end gap-2">
                                                ...
                                                <a href="{{ route('pathways.summary', $pathway) }}" class="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded-md text-white bg-biru-dongker-800 hover:bg-biru-dongker-900" title="{{ __('View Summary') }}">
                                                    {{ __('View Summary') }}
                                                </a>
                                            </div>
                                        </td>
                ...
                @else
                    <div class="text-center py-12">
                        ...
                        <p class="mt-1 text-sm text-gray-500">
                            {{ __('No pathways match the selected filters.') }}
                        </p>
                    </div>
                @endif
            </div>

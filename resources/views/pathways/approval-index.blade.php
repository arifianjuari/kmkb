    ...
    <div>
        <div class="flex justify-between items-center mb-6 flex-wrap gap-4">
            <div class="flex items-center gap-3 flex-shrink-0">
                <h2 class="text-2xl font-bold text-gray-900 whitespace-nowrap">{{ __('Pathway Approval') }}</h2>
                <button
                    type="button"
                    ...
                    onclick="const p = document.getElementById('approval-help'); if (p) { p.classList.toggle('hidden'); }"
                    aria-label="{{ __('What is Pathway Approval?') }}"
                    title="{{ __('What is Pathway Approval?') }}"
                >
                    i
                </button>
            </div>
        ...
        <div id="approval-help" class="mb-4 hidden text-xs text-gray-700 bg-biru-dongker-200 border border-biru-dongker-300 rounded-md p-3">
            <p class="mb-2">
                <span class="font-semibold">Pathway Approval</span> adalah proses review dan persetujuan pathway oleh Medical Committee. Pathway yang sudah selesai dibuat perlu direview dan disetujui sebelum dapat digunakan untuk patient cases.
            </p>
            <ul class="list-disc list-inside space-y-1 ml-2">
                <li>Review pathway yang berstatus <strong>Review</strong></li>
                <li>Approve atau reject pathway dengan menambahkan komentar</li>
                <li>Lihat history approval untuk tracking perubahan</li>
                <li>Pathway yang approved dapat digunakan untuk patient cases</li>
            </ul>
        </div>
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-lg mb-6">
            <div class="px-4 py-5 sm:p-6">
                <form method="GET" action="{{ route('pathways.approval.index') }}" class="space-y-4">
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-4">
                        ...
                        @if($q || $status != 'review')
                            <div class="flex items-end">
                                <a href="{{ route('pathways.approval.index') }}" class="w-full inline-flex justify-center items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">
                                    {{ __('Clear') }}
                                </a>
                            </div>
                        @endif
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-lg">
            <div class="px-4 py-5 sm:p-6">
                <h3 class="text-lg font-medium text-gray-900 mb-4">{{ __('Pathways Pending Approval') }}</h3>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach($pathways as $pathway)
                                    <tr class="{{ $pathway->status === 'review' ? 'bg-yellow-50' : '' }}">
                                        ...
                                        <td class="px-6 py-2 whitespace-nowrap text-right text-sm font-medium">
                                            <div class="flex justify-end gap-2">
                                                ...
                                                <a href="{{ route('pathways.approval', $pathway) }}" class="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded-md text-white bg-biru-dongker-800 hover:bg-biru-dongker-900" title="{{ __('Review & Approve') }}">
                                                    {{ __('Review') }}
                                                </a>
                                            </div>
                                        </td>
                ...
                @else
                    <div class="text-center py-12">
                        ...
                        <h3 class="mt-2 text-sm font-medium text-gray-900">{{ __('No pathways pending approval') }}</h3>
                        <p class="mt-1 text-sm text-gray-500">
                            {{ __('All pathways have been reviewed or no pathways match the selected filters.') }}
                        </p>
                    </div>
                @endif
            </div>

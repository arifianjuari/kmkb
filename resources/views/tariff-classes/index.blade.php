    ...
    <div>
        <div class="flex justify-between items-center mb-6 flex-wrap gap-4">
            <div class="flex items-center gap-3 flex-shrink-0">
                <h2 class="text-2xl font-bold text-gray-900 whitespace-nowrap">{{ __('Charge Classes') }}</h2>
                <button
                    type="button"
            ...
            <div class="flex items-center space-x-2">
                ...
                <button type="button" onclick="document.getElementById('importModal').classList.remove('hidden')" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">
                ...
                <a href="{{ route('tariff-classes.create') }}" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-biru-dongker-800 hover:bg-biru-dongker-900">
                    {{ __('Add New Charge Class') }}
                </a>
                @endif
            </div>
        </div>
        <div id="tariff-classes-help" class="mb-4 hidden text-xs text-gray-700 bg-biru-dongker-200 border border-biru-dongker-300 rounded-md p-3">
            <p class="mb-2">
                <span class="font-semibold">Charge Class</span> adalah pengelompokan kelas tarif yang akan dipakai saat pricing.
            </p>
            <div class="mb-2">
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-lg">
            <div class="px-4 py-5 sm:p-6">
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            <div class="flex items-center gap-2">
                                                ...
                                                @if(!auth()->user()?->isObserver())
                                                <a href="{{ route('tariff-classes.edit', $tariffClass) }}" class="inline-flex items-center justify-center w-9 h-9 rounded-md border border-gray-300 text-biru-dongker-800 hover:bg-biru-dongker-200 focus:outline-none focus:ring-2 focus:ring-biru-dongker-700" title="{{ __('Edit') }}" aria-label="{{ __('Edit') }}">
                                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-5 h-5">
                                                ...
                                                <form action="{{ route('tariff-classes.destroy', $tariffClass) }}" method="POST" class="inline" onsubmit="return confirm('{{ __('Are you sure you want to delete this charge class?') }}')">
                                                    @csrf
                                                    @method('DELETE')
                ...
                @else
                    <p class="text-gray-600">{{ __('No charge classes found.') }}</p>
                @endif
            </div>
        </div>
    </div>
...
<div id="importModal" class="hidden fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg shadow-xl max-w-md w-full mx-4">
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
            <h3 class="text-lg font-medium text-gray-900">Import Charge Classes</h3>
        ...
        <form action="{{ route('tariff-classes.import') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="px-6 py-4">
                <div class="mb-4">
                    ...
                    <a href="{{ route('tariff-classes.template') }}" class="inline-flex items-center text-sm text-biru-dongker-800 hover:text-biru-dongker-900">

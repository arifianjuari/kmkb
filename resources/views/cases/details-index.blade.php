    ...
    <div>
        <div class="flex justify-between items-center mb-6 flex-wrap gap-4">
            <div class="flex items-center gap-3 flex-shrink-0">
                <h2 class="text-2xl font-bold text-gray-900 whitespace-nowrap">{{ __('Case Details') }}</h2>
                <button
                    type="button"
                    ...
                    onclick="const p = document.getElementById('details-help'); if (p) { p.classList.toggle('hidden'); }"
                    aria-label="{{ __('What is Case Details?') }}"
                    title="{{ __('What is Case Details?') }}"
                >
                    i
                </button>
            </div>
        ...
        <div id="details-help" class="mb-4 hidden text-xs text-gray-700 bg-biru-dongker-200 border border-biru-dongker-300 rounded-md p-3">
            <p class="mb-2">
                <span class="font-semibold">Case Details</span> menampilkan daftar patient cases. Pilih case untuk melihat dan mengelola detail services yang dilakukan pada kasus tersebut.
            </p>
            <ul class="list-disc list-inside space-y-1 ml-2">
                <li>Lihat services yang dilakukan pada setiap case</li>
                <li>Tambah, edit, atau hapus case details</li>
                <li>Generate planned steps dari pathway</li>
                <li>Mark services sebagai performed</li>
            </ul>
        </div>
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-lg mb-6">
            <div class="px-4 py-5 sm:p-6">
                <form method="GET" action="{{ route('cases.details.index') }}" class="space-y-4">
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-4">
                        ...
                        @if($q || $pathwayId)
                            <div class="flex items-end">
                                <a href="{{ route('cases.details.index') }}" class="w-full inline-flex justify-center items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">
                                    {{ __('Clear') }}
                                </a>
                            </div>
                        @endif
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-lg">
            <div class="px-4 py-5 sm:p-6">
                <h3 class="text-lg font-medium text-gray-900 mb-4">{{ __('Select Case to View Details') }}</h3>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('Admission Date') }}</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach($cases as $case)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            {{ $case->admission_date ? $case->admission_date->format('d M Y') : '-' }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <a href="{{ route('cases.details', $case) }}" class="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded-md text-white bg-biru-dongker-800 hover:bg-biru-dongker-900">
                                                {{ __('View Details') }}
                                            </a>
                                        </td>
                                    </tr>

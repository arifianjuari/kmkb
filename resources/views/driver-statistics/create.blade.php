...
<div class="max-w-7xl mx-auto">
    <div>
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-bold text-gray-900">{{ __('Add New Driver Statistic') }}</h2>
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-lg">
            ...
            <div class="px-4 py-5 sm:p-6">
                <form action="{{ route('driver-statistics.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-12">
                            ...
                            <div class="mt-1">
                                <select id="allocation_driver_id" name="allocation_driver_id" required class="py-2 px-3 block w-full border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-biru-dongker-700 focus:border-biru-dongker-700">
                                    ...
                                    @foreach($allocationDrivers as $ad)
                                        <option value="{{ $ad->id }}" {{ old('allocation_driver_id') == $ad->id ? 'selected' : '' }}>{{ $ad->name }} ({{ $ad->unit_measurement }})</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-span-12 md:col-span-6">
                            ...
                            <div class="mt-1">
                                <input type="number" id="value" name="value" step="0.0001" min="0" value="{{ old('value') }}" required class="py-2 px-3 block w-full border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-biru-dongker-700 focus:border-biru-dongker-700">
                                @error('value')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>

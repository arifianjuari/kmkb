                ...
                <form action="{{ route('driver-statistics.update', $driverStatistic) }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-12">
                            ...
                            <div class="mt-1">
                                <select id="cost_center_id" name="cost_center_id" required class="py-2 px-3 block w-full border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-biru-dongker-700 focus:border-biru-dongker-700">
                                    ...
                                    @foreach($costCenters as $cc)
                                        <option value="{{ $cc->id }}" {{ old('cost_center_id', $driverStatistic->cost_center_id) == $cc->id ? 'selected' : '' }}>
                                            @if($cc->building_name)
                        ...
                        <div class="col-span-12 md:col-span-6">
                            ...
                            <div class="mt-1">
                                <input type="number" id="value" name="value" step="0.0001" min="0" value="{{ old('value', $driverStatistic->value) }}" required class="py-2 px-3 block w-full border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-biru-dongker-700 focus:border-biru-dongker-700">
                                @error('value')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
